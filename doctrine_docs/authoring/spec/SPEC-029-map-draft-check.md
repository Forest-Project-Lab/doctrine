...
黙って緑にしないことを務めとする。検証の道が無いものは「機械検証不能」として数え、
道が在るのに合わないものは所見にする。次の形はいずれかへ必ず落ちる。

- `provenance` の欠落・非配列・空・非 dict の要素（値を担う実体に限る。アンカーは除く）
- 絶対パス・`..`・作業木の外を指すシンボリックリンク
- 錨（行番号か 4 文字以上の「引用」）の無い locator
- 行番号と引用が食い違う形（引用が示された行の近傍に無い）
- `verdict: silent` と言いながら本文に在る引用
- `review_status` の欠落（値を担う実体）と、入れ子に潜む `confirmed`
- 一覧の非 dict 要素・id の重複・実在しない要素を指す Flow
- 依存グラフの辺の和文の言い換え（`依存グラフ`・`被影響` ほか）
- 検証対象でない接頭の出所（`--repo-prefix` が無いときは模型の多数派を対象とみなす）
- 深すぎる入れ子（形の所見にする。「対象が無い」の終了コードに化かさない）
...
- **語彙と形の正本は共有コア `_model`（SPEC-031）が持つ。** 以前はこのスクリプトが写しを持っており、リンタ・描き手・門で三重になる形だった（ADR-163 決定8）。
- 掛ける先は、当面**描いた JSON**とする（MODEL 型の .md から `render-projection.py model` で描いたもの、または起草の技能が書いた下書き）。門を .md の側へ移すかは、doctrine-lens 側の読み方に影響するので issue #294 で相談してから決める（ADR-163 の帰結）。
