...
型のテンプレートが20個、投影の雛形が1個あること。各テンプレートの既定値が登録簿と一致すること。SPEC が必須4節の見出しを持つこと。PROC が4節（目的と発動条件・前提・手順・切り戻し）の見出しを持つこと。DECIDED・WATCH が `review_by` 欄を持つこと。投影テンプレートが冒頭の注記を持つこと。GLOSSARY の雛形が20個の承認語を持つこと。以上を TEST-017 が確認する。
