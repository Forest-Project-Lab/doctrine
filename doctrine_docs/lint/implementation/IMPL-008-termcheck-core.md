...
- `_mask_approved_compounds` は、文字列照合の前に、組み込みの複合語（`入出力` や `現在形`）に加え、登録簿の必須節の完全な名（`_registry_section_names`。ADR-135）、辞書の固有名の行（`proper_nouns`）、禁止同義語を包含する登録済み承認語を、長い語から先に伏せる（ADR-018）。覆いの外に素のまま現れた禁止同義語だけを照合に残す[R10]。登録簿が読めない環境では節名の源だけが空へ退く。
