...
- 検出コードの一覧は次のとおり。フロントマターと登録簿に関わる `MISSING_FRONTMATTER`・`MISSING_KEY`・`EMPTY_KEY`・`BAD_STATUS`・`UNKNOWN_TYPE`・`ID_FILENAME_MISMATCH`・`BAD_FILENAME`・`TYPE_LOCATION_MISMATCH`・`DOMAIN_PATH_MISMATCH`・`BAD_LLM_CONTEXT`・`RESEARCH_HAS_DECISION`・`SPEC_MISSING_SECTION`・`SPEC_EMPTY_SECTION`・`MISSING_TRACE`・`STRAY_DOCUMENT`・`ARCHIVED_LOCATION_MISMATCH`・`ICD_DEP_VIOLATION`・`ICD_DEP_UNVERIFIED`、用語側の `BANNED_SYNONYM`・`CALQUE`・`CALQUE_WORDTRAP`・`UNDEFINED_TERM`・`GLOSSARY_PARSE_ERROR`。
