...
`plugin/scripts/inject-contract.py`（`estimate_tokens`・`_build_sections`・`_assemble`・`_trim_to_fit`・`_load_audit_summary`・`_plugin_root_cache_candidates`・`_frontmatter.sanitize_inline`（ADR-040）・`_fact_lines`/`_facts_lines`（ADR-043 要点行の抽出と描画））。共有の `_registry`・`_frontmatter` を import する。監査要約の候補順はプロジェクトスコープ先・旧プラグインroot配置は後方互換の最後（ADR-037、#69）。`gov-heartbeat.py` の `_audit_summary` と同じ順序に揃える。
