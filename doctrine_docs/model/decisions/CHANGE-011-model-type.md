...
- 起草の技能 `system-map-draft` を、MODEL 型の .md を統治木の中へ起草する形へ改めた
  （SPEC-016 の記述も追随）。検収は四つの門を順に回す（構造 → 描画 → 出所 → M 層）。
- 実例として `MODEL-001`（doctrine 自身の最小の意味モデル）を同梱した。**網羅は主張しない**
  ——型と門が端から端まで働くことを示すための最小の模型である。
