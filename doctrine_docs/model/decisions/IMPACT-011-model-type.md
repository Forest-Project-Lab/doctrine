...
### 追補（2026-08-14。ADR-165・EXT-007）

doctrine-lens が実例の投影を自分の門へ掛け、**器に適合していないこと**を報せた。こちらで
再現して確かめ、写しをやめた。

- 実装: `plugin/schemas/system-map-gold-model-0.1.json`（固定した器の一枚を同梱）と
  `_model.py`（必須欄・段の必須欄・出所の必須欄・語彙をそこから導く）。手で並べた表を落とした。
- 宣言: ADR-165・EXT-007（同梱の一枚を指紋で見張る）・SPEC-031・TEST-031。
- 追随: 雛形 `model.md.tmpl`・起草の手引き `model-shape.md`・実例 MODEL-001（シナリオの
  六欄と、要素の `realized_by`／`realization`）。
- 試験: 器からの導出を凍らせる五件を足した（`SchemaDerivationTest`）。

実例の門の実測（2026-08-14）: リンタ所見 0 件 / 描画 成功 / 出所の門は **出所 12 件・所見 0 件・
機械検証不能 0 件**。M 層は**その後に回した** —— 上流の固定点の複製で 合格 14／不合格 0、上流の現行版では
形の門が 8 件で落ちた（Scenario の必須欄）。器へ揃えた後は 合格 11／不合格 0／空虚 6 と
到達の門も合格（到達可能な要素 4 件）である。
