...
- `SUBDOMAIN_KINDS`: ドメインの**種類**の語彙を `core`・`supporting`・`generic` の三語で持つ（ADR-092）。ドメインの名ではなく、その区画へ何を投じるかの種類である。**語彙の正本はここだけに置き、呼び手は三語を自分の実装へ書き写さない。** この項に既定値は無く、型からも導かない —— 省略は「未分類」である。**投資の判断そのものはこの語彙が持たない**: 中核は要求の「優先度」が言い、一般は `NONGOAL` が言い、委ねたことは `ADR` が言う。`review_by` の要求も再点検周期も持たない（項の上書きは機械に見えないので、期限を課しても守られたことを確かめられない）。
...
- 位置づけ（`status`）の正本は、平文のフロントマターである。**イベント台帳と reducer から導出しない**（ADR-093。非目標 第18項）。登録簿は語彙と許可表を持つだけで、状態の履歴を持たない。自己申告を縛るのは監査の証拠の検査であり、正本の置き換えではない。
