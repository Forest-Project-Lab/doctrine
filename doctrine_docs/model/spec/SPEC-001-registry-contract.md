...
- `required_keys(type)`: 必須キーの列を返す。DECIDED と WATCH には `review_by` を加える。**必須キーの規則の正本はここだけである**（確定事実1。ADR-106）—— 以前はリンタが `REQUIRED_KEYS_L2` と `REQUIRED_REVIEW_BY_TYPES` を自前で組み合わせており、同じ規則が二箇所に在った（そして正本の側は誰にも呼ばれていなかった）。**段（level）の口は落とした** —— 受け取って無視していたので、何も決めない口を公開しない。
...
- 段の梯子（どの鍵がどの段で意味を持つか）は**説明であって規則ではない** —— `depends_on`・`impacts` は段3から、`canonical_for` は段4から意味を持つが、**段2でそれらを書くのは違反ではない**（まだ意味を持たないだけである）。禁じるものが無いので検める対象が無く、表も持たない（ADR-106。以前は `LEVEL3_KEYS`・`LEVEL4_KEYS` が在ったが、消費者ゼロのまま古びていた）。
