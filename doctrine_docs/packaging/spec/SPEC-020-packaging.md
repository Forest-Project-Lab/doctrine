...
- **試験が実時計に依っていたら、メタの受入で失敗とする**（ADR-094。WATCH-001 第11項）。対象は「時計を固定する口（`--today`）を持つ呼び出しを起動し、かつ固定日の `generated_at` を埋め込んでいるクラス」であり、その固定を渡していなければ失敗とする。**判定はクラス単位で、同じモジュール内の基底クラスまで辿る** —— 呼び出しは基底の補助が起動し、固定日は派生の側に書かれるので、ファイル単位では掴めない（掴めないことを実測した。落ちた当日でもファイル単位の検めは通った）。対象は口の有無で決め、**免除の一覧を持たない**（呼び出しが後から口を得れば、その試験も自動で対象に入る）。
