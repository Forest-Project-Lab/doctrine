...
- 証跡の節が黙って消えないこと。走行の合否には影響しないので、消えても試験は緑になりうる
  —— だから節の存在そのものを受入で見る。
- 統治対象の内容（プラグインの置き場所・作業ディレクトリ・統治木の名前）が刷られるように
  ならないこと。刷る項目を足すときは、この観点に照らす。
- 証跡の取得の失敗が、走行の合否や終了符号を変えないこと。

## 合否基準

`plugin/tests/test_meta.py` の `TestRunProvenance` の全ケースが合格すること。
節の五項目が上の順で並び、環境の項目が実際の値を持ち、`git` の届かない場所でも走行が
落ちず、統治対象の内容が刷られないことを合格とする。
