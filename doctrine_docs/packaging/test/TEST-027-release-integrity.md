...
- ADR-096: 設定の見張りの検査が、(1) `hash` のアンカーと指紋の行が揃っていれば無音、(2) **アンカーが無ければ**違反、(3) **検査が `exists`** なら違反、(4) **指紋の行が無ければ**違反、(5) 設定が無い木では無音、(6) **本文に `hash` の語が在るだけでは通らない**（読むのは `検査:` の行。この取り違えで一度飾りになった）ことを凍結する。`ConfigAnchorTest` が確認する。**歯止め自身の実効を実測してある**（2026-08-03）: 実物の木でアンカーを消す・検査を落とす・指紋を消すの三通りを当て、それぞれ別の違反文で落ち、戻すと通った。
