...
- `to_json()` → `{root, nodes, edges, dup_ids, parse_warnings}`。**節点は隠さない**（ADR-087）。組み立てが節点へ入れた項をすべて返す。白名簿を持たない —— 以前は八項に絞っており、正本がどこにも無いまま組み立てと別々に手で保つ形になっていて、実際にずれた（組み立てが四項を足した後も白名簿は八項のままで、必須項の `title` は最初から集められてさえいなかった）。
  - 返る項: `id`・`title`・`path`・`type`・`domain`・`owner`・`status`・`depends_on`・`impacts`・`canonical_for`・`superseded_by`・`updated`・`review_by`・`llm_context`・`subdomain`・`sources`・`reproducible`。組み立てが項を足せば、そのまま返る。
  ...
  - **必須キー8個（`id`・`title`・`type`・`domain`・`status`・`owner`・`updated`・`sources`）はすべて節点に在る**（ADR-098 で凍結）。`owner` は集めていない最後の一つで、題名（ADR-087）・出所（ADR-097）に続く**三件目**だった。**四件目を作らないために、必須キーの全部が節点に在ることを機械が検める。**
