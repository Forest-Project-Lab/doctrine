...
- 読めない値が他のガードを開かないこと（2026-08-03 の実測。ADR-102 が「測っていない」と書いた経路を測った）: 一覧で書いた `status` でも **Bash の削除**の逆依存の判定は狂わず（グラフ側で判ずる）、一覧で書いた `type` でも**越境依存**の判定は狂わない（依存先のドメインは置き場所で解く）。**穴が無いことも凍らせる** —— 測っていないままにすると、後から静かに開いても気づけない。なお PostToolUse の突き合わせは `Edit|MultiEdit` だけを扱う（`Write` は PreToolUse が全文を事前判定済み。仕様の入出力にそう書いてある）ので、そこは対象にしない。
