...
- 24 検査の重大度は固定とする（ICD-005 の表のとおり。`AUDIT_CHECKS` の名前は 26 個で、逆孤児の二種と未登録/影がそれぞれ一つの検査を二つの名前で持つ）。`top_findings` は error を優先し、上限 20 件とする。
...
- コードと仕様の追跡（trace_*、ADR-056）: 仕様の文書が `## 実装の指紋` の節に `- sha256:<64桁>` を持つときだけ効く。節を持つ仕様が一つも無ければ、コードの走査そのものを行わない（使っていない機能の費用を払わせない）。走査は `_tracescan`（SPEC-026）に委ね、統治木の親を根とする。挙げるのは五つ — 印の対応付けの誤り（error）・実在しない id を指す注釈（error）・廃止/置換済みの id を指す注釈（warn）・記録した指紋との食い違い（warn）・記録があるのに範囲が無い（warn）。「根拠を持たないコード」は挙げない（注釈は任意であり原理的に判じられない。ADR-054 の既知の限界）。`[R3][R4]`
