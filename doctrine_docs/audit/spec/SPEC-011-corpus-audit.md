...
- 孤児の陳腐化は `updated` の古さだけで判ずる。**`review_by` の超過を陳腐化の引き金にしない**（ADR-149）。二つは測っているものが違う —— `review_by` は「予定した見直しの日が来た」という暦の出来事、`updated` の古さは「誰にも引かれないまま放置されている」という状態であり、孤児が測るのは後者だけである。`review_by` の信号は消えない（専用の検査 `review_by_overrun`（WARN）が従来どおり届ける）。二重に数えると、日付が進んだだけで削除候補が湧く。`[R8]`
