...
この設計書自体が、本体系の文章規範（第5.7節）に従って書かれている。すなわち一段落一トピック、推量と断定の分離、空句の排除、同一概念は同一語で表記、「100%保証」を主張しないという規範である。実装される文書も同じ規範に従わせる。
...
ADR（type=ADR）の許可値は {proposed, accepted, superseded, deprecated}、それ以外の型は7語（accepted を除く）とする。これは意図的で境界の明確な例外であり、リンタは型ごとの許可リストで `status` を検証する。
...
- **検証規則（`docs-linter.py` が実装。第8章）**: 必須キーの存在（id/title/type/status/owner/updated/sources）、`status` が型別許可リストに含まれる、日付が ISO 8601、`id` がファイル名の型コード+連番と一致、`type` とフォルダが第5.1節表と整合、`llm_context ∈ {always,task,never}`、`depends_on`/`impacts`/`supersedes`/`superseded_by` が実在 ID を指す（dead link 検出）、型規律の軽い検査（例：RESEARCH に「## 決定」見出しがあれば警告）。
...
| 3 | リンタが誤検出で邪魔 | 規則が厳しすぎる | 必須は7項目に絞る。`continueOnBlock` で修正余地を残す。型規律は警告レベル（第5.3, 8.3節）。 |
