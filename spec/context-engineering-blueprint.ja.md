---
id: DESIGN-001
title: LLM開発のための情報統治 Claude プラグイン（Skills + Hooks）設計書
type: ARCH
status: current
owner: （受領者が記入）
created: 2026-06-24
...
sources:
  - LLM開発情報管理体系.md（アップロード）
  - compass_artifact（アップロード）
...
# 設計書｜LLM開発のための情報統治 Claude プラグイン（Skills + Hooks）
...
読み順は次のとおりとする。第1〜4章で「何を・なぜ作るか」を確定する。第5章で二つの調査文書のマージ判断（正本仕様の根拠）を示す。第6章が **正本リファレンス**（Skills と Hooks が共通で実装する単一の正本）である。第7〜10章が実装仕様（Skills／Hooks／スクリプト／パッケージング）である。第11章がビルド順序の具体的指示である。第12〜14章が検証・破綻予測・チェックリストである。付録にテンプレートと出典を置く。

この設計書自体が、本体系の文章規範（第6.7節）に従って書かれている。すなわち一段落一トピック、推量と断定の分離、空句の排除、同一概念は同一語で表記、「100%保証」を主張しないという規範である。実装される文書も同じ規範に従わせる。
...
2. LLM の典型的失敗（廃案復活・未決の既決化・仕様と実装の取り違え・古い前提の採用）は、いずれも「情報の位置づけ（現行か、廃案か、未決か、根拠か）が文書構造に書かれていない」ことに起因する。ゆえに設計の主眼は、量を捌くことではなく、各情報片に**位置づけ・出所・依存・現行性**を付与して追跡可能にすることに置く。
3. コンテキストは「多いほど良い」ではない。リポジトリのコンテキストファイル（CLAUDE.md / AGENTS.md）を肥大させると、コーディングエージェントの成功率はむしろ下がり、推論コストは増える（ETH Zürich & LogicStar.ai, arXiv:2602.11988, 2026）。ゆえに常時投入は「最小の、エージェントが他から知り得ない非自明な事実」に絞る。CLAUDE.md は知識の集積ではなく、現行文書への入口だけを示す**最小限の案内**にする。
...
- **柱2（単一の正本仕様）**: Skills と Hooks は、別々の規則を持ってはならない。第6章の正本リファレンス（文書型・ステータス統制語彙・メタデータ・フォルダ・LLMコンテキスト区分・ライフサイクル・文章規範）を、両者が同一の真実として参照する。リンタの判定規則と、Skill が文書を作る規則は、同じ表から導かれる。
...
- 無効な参照 ID（dead link）と、依存グラフの破れ（リンタとフルリポジトリ監査が検出）。
- アーカイブ済み文書への直接編集、確定済み ADR の改変（PreToolUse ガードが拒否）。
- 廃案・未決・禁止事項の常時不在による退行と既決化（SessionStart 注入と Regression Watchlist で予防）。
...
ビルドの成否は、抽象的な「管理できている感」ではなく、**7つの観点それぞれについて、それを担保する Skill・Hook・スクリプト・テンプレートを指させるか**で判定する。指させれば達成、指させなければ未達とする。

| # | 観点 | 達成条件（何を指させればよいか） |
|---|---|---|
| 1 | 見つけやすさ | フォルダ・命名規則・Overview 索引で目的の文書に到達できる。`doc-author` が型コード命名を強制し、`docs-audit` が孤立文書を検出する。 |
| 2 | 現行性 | 各文書の `status` で現行が判別できる。リンタが統制語彙を強制し、監査が `review_by` 超過の 現行文書を一覧化する。 |
| 3 | 追跡性 | Requirement→Spec→実装→Test→ADR を `depends_on`/`impacts` でたどれる。`change-impact` がトレーサビリティを生成・点検する。 |
| 4 | 変更に耐える | 変更時に更新すべき文書集合を依存リンクから列挙できる。`change-impact` の14ステップと、リンタの dead-link 検出。 |
| 5 | LLMに強い | 常時投入が現行・禁止・非目標・未決に限定され、廃案本文は渡らない。SessionStart 注入と `llm-context-pack` と PreToolUse アーカイブガード。 |
| 6 | 認知負荷 | 一文書一目的、粒度が揃い、抽象語でぼかさない。`doc-review` の k16shikano ルーブリック。 |
...
- **A. 正本仕様の確定（何を実装するか）**
  - A1 二文書のマージ判断（ステータス語彙・メタデータ・フォルダ・文書型・変更フロー）
...
- **Answer（答え）**: 位置づけで分離する文書型体系と、出所・依存・現行性を必須メタデータ化する運用を、**Hook（決定論的強制）と Skill（手続き的支援）に役割分担**して実装する。両者は単一の正本仕様（第6章）を参照する。常時投入は最小限の案内と最小の確定事実に限定する。これらを単一プラグインに束ね、`/plugin install` で配布し、init Skill で各リポジトリへ最小配置する。詳細を第5章以降で示す。

---

## 5. 統合方針（二文書のマージ判断）

二つのアップロード文書は、同じ主題の二版である。一方（`LLM開発情報管理体系.md`）は概念枠組みが網羅的で、24テンプレートと6観点の成熟度を持つ。他方（`compass_artifact`）は実務に即して精密で、7語の統制語彙・リッチなメタデータ・14ステップの変更フロー・ETH 研究の限界提示を持つ。正本仕様は、原則として後者の厳密さを採り、前者の網羅性で補う。判断と根拠を以下に固定する。

| 論点 | 採用 | 根拠 |
|---|---|---|
| ステータス語彙 | **7語の統制語彙**（draft/proposed/current/deprecated/superseded/archived/open）を採用。`active` は使わず `current` に統一。 | 統制されており、現行と提案中と未決と退役を区別できる。リンタが機械検証しやすい。 |
| メタデータ | **リッチなスキーマ**（id/title/type/status/owner/created/updated/review_by/sources/depends_on/impacts/supersedes/superseded_by/llm_context）。必須は絞る（第6.3節）。 | 依存・変更・退行管理に必要。ただし全項目必須は形骸化を招くため成熟度別に分ける。 |
| フォルダ構成 | **11フォルダのハイフン版**（/00-overview … /99-archive + ルート AGENTS.md）。 | 決定・実装・テスト・運用を分離。ハイフンはパスで安全。位置づけと型でトップレベルが一意。 |
| 文書型 | **統合した単一表**（第6.1節）。位置づけの列を持つ分類表を骨格に、機能群（常時/Semantic/Episodic/運用変更/アーカイブ）で整理。 | 両文書の型を漏れなく一意化し、リンタと `doc-author` が同じ表を参照できる。 |
| 変更フロー | **14ステップ**（第6.6節）。更新順序は ADR→Spec→実装→テスト→LLM→廃案整理。 | 影響分析・更新順序・退行チェックを含み、手戻りを減らす。 |
| Diátaxis の適用範囲 | 製品リファレンス文書のみ。**ADR・調査・Runbook には適用しない**（ライフサイクル段階で整理）。 | Diátaxis 自体が内部プロセス文書を対象外とする。プロセス文書は「どの段階が生んだか」で整理する。 |
| 常時投入の重さ | **最小化**。CLAUDE.md/AGENTS.md は最小限の案内。常時投入は確定事実・非目標・廃案事実・未決・用語・Watchlist 要点に限定。 | 肥大を避けるため（前提3）。 |
| 日本語規範 | **k16shikano 規範**を採用・改変・除外に分けて取り込む（第6.7節）。 | LLM 生成物のレビュー基準として適合する。固有名は散文で一般名、参照 ID はメタデータへ。 |

ADR のステータスについて一点補足する。グローバルな `status` 統制語彙は7語だが、ADR（type=ADR）に限り Nygard 慣習の `accepted` を許可する。すなわちリンタは **型ごとの許可リスト**で `status` を検証する。ADR の許可値は {proposed, accepted, superseded, deprecated}、それ以外の型は7語（accepted を除く）とする。これは意図的で境界の明確な例外である。
...
`status` は以下の7語のみを使う（ADR の `accepted` を含めて型別許可リストで検証）。これ以外を使わない。自由語を増やすと見つけやすさと機械検証が崩れる。
...
**形骸化を防ぐ設計**: `status` と `updated` は LLM に更新を代行させ、人間は承認のみ。`review_by` 超過の 現行文書を `docs-audit` が「現行性疑い」として一覧化する。
...
└── docs/
    ...
    └── 99-archive/           # 退役文書。現行と物理的に分離。RAG/検索から除外
```
...
- **タスク別投入（llm_context: task）**: 当該機能の現行 Spec、関連 ADR、関連 Implementation Note。必要なときだけ読み込む（progressive disclosure）。文脈を肥大させない。
...
## 7. 実装仕様 — Skills（手続きと判断）

Skill は「必要なときに読み込む専門知識（on-demand）」である。description が常時コンテキストに載り、本文はトリガー時のみ読み込まれる。**description がトリガーそのもの**であるため、三人称・主要ユースケース先頭・ユーザーが実際に言う語句の列挙で書く。Claude は単純な一手で済むタスクには Skill を使わない傾向（呼ばれにくさ）があるため、description は少し「押しが強い」書き方にする。
...
### 7.1 `docs-system-init`（配置）｜Level 0→2

- **description 要旨**: 「リポジトリに LLM 向け情報統治を導入する。`/docs` 構造の最小セットと、最小限の AGENTS.md/CLAUDE.md、ステータス統制語彙、メタデータ規約、リンタ・監査スクリプト、Hook 設定を配置する。新規リポジトリの初期化、既存リポジトリへの体系導入、`docs` の標準化、AGENTS.md の整備、を求められたら必ず使う。」
- **手続き**:
  1. 既存の `docs/` と AGENTS.md/CLAUDE.md を検出し、**破壊しない**（あれば差分提案、なければ新規）。
  2. 既定（Level 1）で最小5ファイル + 最小限の案内のみ配置する（第6.4節）。フル11ツリーは作らない。
  3. プロジェクトに Hook を有効化する。プラグインとして配布する場合はプラグインの `hooks/hooks.json` が効くので追加不要。プレーン `.claude/` 運用の場合は `.claude/settings.json` に第8章の Hook を書き込む。
  4. リンタ・監査スクリプトを配置（プラグイン同梱なら参照のみ。スタンドアロンなら `.claude/scripts/` へコピー）。
  5. 配置後、第14章の「採用チェックリスト」を出力し、次のレベルへの移行条件を示す。
- **参照ファイル**: `references/levels.md`（Level 0–5 の状態・移行条件・失敗しやすい点）、`references/folder-layout.md`（第6.4節の詳細）。
- **スクリプト**: `scripts/scaffold.py`（最小配置。既存ファイル非破壊）。
- **対応 Hook**: 配置後は SessionStart 注入、PreToolUse ガード、PostToolUse リンタが効く。

### 7.2 `doc-author`（型付き文書の作成・更新）｜Level 1→2
...
- **手続き**:
  1. 第6.1節の表から型を決め、フォルダと既定 status と llm_context を引く。フォルダが無ければ生成する（遅延生成）。
  2. ファイル名（`<型コード>-<連番>-<主題>.md`）と `id` を一致させる。
  3. 付録のテンプレートからフロントマター+本文骨格を生成し、「書く/書かない」規則を適用する。
  4. 価値の変化の昇格基準（第6.6節）に該当する場合は、元文書を残しつつ昇格先（ADR など）を作り、`sources`/`supersedes` でリンクする。
...
### 7.3 `doc-review`（文章規範＋位置づけレビュー）｜Level 2→3

- **description 要旨**: 「文書または LLM 生成物に、日本語技術文書規範（k16shikano ベース）と位置づけレビューを適用する。文章をレビューする、LLM の出力を点検する、抽象的すぎる記述を直す、推量と断定の混在を直す、未決が既決として書かれていないか確認する、といった依頼で必ず使う。」
- **手続き**:
  1. 第6.7節のレビュー観点6項目を順に適用し、違反箇所を列挙する。
...
- **参照ファイル**: `references/jp-writing-rubric.md`（第6.7節の全規則と良い例・悪い例）。
...
### 7.4 `change-impact`（変更・依存・影響分析）｜Level 4
...
- **手続き**: 第6.6節の14ステップを実行する。`depends_on`/`impacts` をたどり、影響文書・実装・テストを列挙し、アーキテクチャ的有意性テストで ADR の要否を判定し、更新順序（ADR→Spec→実装→テスト→LLM→廃案整理）を守り、旧版を Archive へ移し `superseded_by` を付ける。Impact Analysis 文書を出力する。
...
### 7.5 `regression-guard`（退行防止）｜Level 4
...
- **手続き**:
  ...
  3. 退行なら復旧手順（第6.6節）を起動する。Watchlist へ監視点を追加し、可能なら受入テスト化する（例「住所が DB に保存されないこと」）。
...
- **対応 Hook**: SessionStart 注入（Decided Facts/Deprecated/Watchlist 要点を常時投入）が予防の主力。

### 7.6 `llm-context-pack`（コンテキスト・エンジニアリング）｜Level 5
...
- **手続き**:
  1. 第6.5節の3区分に従い、`always` 群（事実・要点のみ）+ タスク対象の現行 Spec + 関連 ADR を集約する。
...
- **スクリプトは stdlib のみ**: pip 依存を作らない（どこでも動くため。第9章）。
...
- PostToolUse は `{"decision": "block", "reason": "..."}` でターンを止める。`"continueOnBlock": true` を付けると、ブロックではなく tool_result として指摘を Claude に返し、Claude が読んで修正・再試行できる。**リンタにはこれを使う。**
...
- **強制する不変条件**: 「LLM に毎回読ませるべき文書」の常時不在を防ぐ。これにより廃案復活・既決化・古い前提の採用を予防する。
- **設定例**:
```json
{
  "hooks": {
...
- **目的**: (a) `/99-archive/**` への書き込み・編集を拒否する（退役文書は不変の証跡。編集は退行を招く）。(b) `/40-decisions/ADR-*.md` の**既存ファイルの改変**（Edit）を拒否する（ADR は追記型。覆すときは新 ADR を作り `superseded_by`）。
...
- **目的**: 文書の書き込み・編集の直後に、触れたファイルのフロントマターと参照整合を検証する。これが「CI でリンタを走らせ、必須メタデータの欠落や無効な参照 ID をビルドエラーとして弾く」の実体である。
- **検証規則（`docs-linter.py` が実装。第9章）**: 必須キーの存在（id/title/type/status/owner/updated/sources）、`status` が型別許可リストに含まれる、日付が ISO 8601、`id` がファイル名の型コード+連番と一致、`type` とフォルダが第6.1節表と整合、`llm_context ∈ {always,task,never}`、`depends_on`/`impacts`/`supersedes`/`superseded_by` が実在 ID を指す（dead link 検出）、型規律の軽い検査（例：RESEARCH に「## 決定」見出しがあれば警告）。
- **ハンドラ / 出力**: `type: command`、`${CLAUDE_PLUGIN_ROOT}/scripts/docs-linter.py`。違反時は `{"decision":"block","reason":"<具体的指摘>","continueOnBlock":true}` を返し、Claude に指摘を見せて修正させる（ターンは止めない）。`docs/` 配下の `.md` 以外は終了コード 0 で素通り。
- **強制する不変条件**: メタデータ品質、ステータス統制、ID 整合、参照整合、型規律。
- **設定例**:
```json
{
  "hooks": {
    "PostToolUse": [
      { "matcher": "Edit|Write|MultiEdit",
        "hooks": [ { "type": "command", "command": "python3 \"${CLAUDE_PLUGIN_ROOT}/scripts/docs-linter.py\"", "continueOnBlock": true } ] }
    ]
  }
}
```

### 8.2 任意 Hook（オプション。必要になってから足す）

- **(4) Bash 安全ガード（PreToolUse, matcher: `Bash`, if: `Bash(rm *)` など）**: `/99-archive` の削除や破壊的コマンドを拒否する。証跡の喪失を防ぐ。汎用安全策。
- **(5) フルリポジトリ監査（手動 / CI / 任意で Stop）**: `docs-audit.py` を `/audit` コマンドまたは CI で実行する。dead link の全件、`review_by` 超過の 現行文書、N 日以上 draft 放置、RESEARCH に決定混入、Watchlist 未保守 を一覧化する。**毎ターンは実行しない**（エージェントを遅くするため）。重い検査は per-turn ではなくここに置く。
...
- **マッチャは大文字小文字を区別する**（`bash` は `Bash` にマッチしない）。ツールイベントは `tool_name` でマッチし、`Edit|Write|MultiEdit` のように `|` 区切りの完全一致を使う。さらに細かい絞り込みは `if`（パーミッション規則構文、例 `Edit(*/99-archive/*)`）で行う。
...
- **冪等・無害**: 非クリティカルな Hook は失敗してもワークフローを止めない設計にする。リンタは `continueOnBlock: true` で修正余地を残す。
- **パス解決**: プラグイン同梱スクリプトは `${CLAUDE_PLUGIN_ROOT}`、プロジェクト設置は `${CLAUDE_PROJECT_DIR}` を使う。

---

## 9. 実装仕様 — スクリプト（決定論的バリデータ）
...
|---|---|---|---|
| `docs-linter.py` | PostToolUse Hook | stdin JSON（tool_input.file_path） | 単一ファイルのフロントマター+参照検証。違反時 JSON block（continueOnBlock） |
...
### 9.2 共通：最小フロントマター・パーサ（stdlib のみ）
...
### 9.3 `docs-linter.py`（中核。参照実装）

PostToolUse から呼ばれ、触れた単一ファイルのみを検証する（高速）。`docs/` 配下の `.md` でなければ即 exit 0。違反は `continueOnBlock` で Claude に返す。
...
def block(reason):
    print(json.dumps({"decision":"block","reason":reason,"continueOnBlock":True}))
    sys.exit(0)
...
def main():
    ...
    # 7) 参照整合（dead link）：全 docs の id 集合と突き合わせ
    repo = path.split("/docs/")[0]
    ids = set()
    for root,_,files in os.walk(os.path.join(repo,"docs")):
        for fn in files:
            if fn.endswith(".md"):
                m,_ = parse_frontmatter(open(os.path.join(root,fn),encoding="utf-8",errors="ignore").read())
                if m.get("id"): ids.add(m["id"])
    for rk in ("depends_on","impacts","supersedes","superseded_by"):
        vals = meta.get(rk, [])
        vals = vals if isinstance(vals, list) else [vals]
        for v in vals:
            if v and v not in ids:
                errs.append(f"{rk} の参照先 '{v}' が存在しません。実在する id を指定してください（dead link）")
    # 8) 型規律（軽い検査）
...
### 9.4 `inject-contract.py`（SessionStart 最小注入。参照実装）
...
├── hooks/
│   └── hooks.json                  # 第8章の Hook（settings.json の hooks と同形式）
├── skills/
...
`hooks/hooks.json` は第8章の Hook をそのまま入れる（`settings.json` の `hooks` オブジェクトと同形式）。パスは `${CLAUDE_PLUGIN_ROOT}/scripts/...` で解決する。
...
### 10.3 CLAUDE.md / AGENTS.md は最小限の案内にする
...
## 11. ビルド順序（受領する Claude への具体的指示）

以下の順で実装する。各段階は前段階の成果物に依存する。

1. **正本リファレンスをコード化する**: 第6章の表（TYPE_FOLDER / STATUS / REQUIRED / llm_context 既定）を `scripts/_frontmatter.py` と `docs-linter.py` のデータ構造として固定する。規則をここに一元化し、以降のコードに二重定義しない。
2. **スクリプトを実装する**: 第9章。まず `_frontmatter.py` → `docs-linter.py` → `inject-contract.py` → `archive-guard.py` → `scaffold.py` → `docs-audit.py` → `dep-graph.py` → `collect-context.py`。標準ライブラリのみ。`docs-linter.py` と `inject-contract.py` は本書の参照実装を出発点にする。
3. **Hook を構成する**: 第8章のコア3種を `hooks/hooks.json` に書く。任意3種はコメントで残し既定無効。
4. **Skill を実装する**: 第7章の6つ。各 `SKILL.md` は description を「押しの強い」三人称で書き、本文を500行未満に保ち、詳細を `references/` に分ける。テンプレートは `templates/`（付録A）。
5. **テンプレートと最小限の案内を用意する**: 付録A・C を `templates/` に置く。
6. **プラグインとしてパッケージ化する**: 第10.1節の構成と `plugin.json`・`hooks.json`。
7. **自己検証する**: 空のテストリポジトリで `docs-system-init` → 最小5ファイルが配置されることを確認。わざと壊した文書（status 不正・dead link・型/フォルダ不一致）を書き、PostToolUse リンタが `continueOnBlock` で指摘することを確認。`/99-archive` への編集が deny されることを確認。SessionStart で最小契約のみが注入されることを確認。
8. **README を書く**: 導入手順、Skill 一覧、Hook 一覧、`/audit` の使い方、成熟度の上げ方（付録の採用チェックリストへリンク）。

実装中に正本仕様へ疑問が出たら、コードを勝手に分岐させず第6章を更新し、両者を一致させる（柱2）。

---

## 12. Toulmin による中心主張の検証
...
- **裏付け（Backing）**: 記録管理（ISO 15489 の真正性・信頼性・完全性・利用可能性）、要求工学（ISO/IEC/IEEE 29148 の追跡性）、ADR 実務（追記型・superseded 管理）、Claude Code の Hook/Skill 機構（決定論的制御と、必要なときに読み込む専門知識）、コンテキスト最小化の経験的知見（arXiv:2602.11988）。
...
## 13. Premortem（このビルドと運用が一年後に破綻するとしたら）

ある出来事がすでに起きたと想定して振り返ると、これから起きる結果の原因を正しく言い当てる力が上がる（prospective hindsight）。本ビルドが破綻した未来を想定し、原因を先回りする。

| # | 破綻シナリオ | 原因 | 対策（設計に織り込み済み） |
|---|---|---|---|
| 1 | Skill が呼ばれない | description が弱く呼ばれにくい | 三人称・「〜の依頼で必ず使う」・実使用語句の列挙（第7章）。導入後に起動率を点検し description を調整。 |
| 2 | エージェントが遅い | Hook が重く毎ターン全件走査 | per-turn は単一ファイル検証のみ。全件監査は `docs-audit`（コマンド/CI）に隔離（第8.3節）。 |
| 3 | リンタが誤検出で邪魔 | 規則が厳しすぎる | 必須は7項目に絞る。`continueOnBlock` で修正余地を残す。型規律は警告レベル（第6.3, 9.3節）。 |
| 4 | CLAUDE.md 肥大で性能低下 | 常時投入を増やした | 最小限の案内厳守。SessionStart は事実・要点のみ注入（第8.1, 10.3節、前提3）。 |
| 5 | メタデータ形骸化 | 必須が多すぎ手動更新が続かない | 必須を4＋3項目に限定。status/updated は LLM 代行＋人間承認。`review_by` 超過を監査が一覧化（第6.3節）。 |
| 6 | フォルダが空のまま増える | 空ツリーを先に作った | init 既定は最小5ファイル。フォルダは初使用時に遅延生成（第6.4, 7.1節）。 |
| 7 | 廃案が現行として復活 | 廃案の位置づけが文書化されず | Deprecated Item + Watchlist + SessionStart 常時投入 + PreToolUse アーカイブガード（第6.6, 8.1節）。 |
| 8 | 調査と決定が混ざる | RESEARCH に決定を書く | 型の分離。リンタが RESEARCH 内の「決定」見出しを警告（第9.3節）。 |
| 9 | 仕様と実装が乖離 | テストがない／変更フロー不遵守 | 受入・退行テスト化、`change-impact` の14ステップ、更新順序の強制（第6.6節）。文書だけでは閉じないと明示。 |
| 10 | プラグインがリポジトリ間で陳腐化 | 各人が手で `.claude/` を持つ | プラグイン配布＋`/plugin install`＋`/reload-plugins`で中央更新（第10章）。 |
...
### 14.1 ビルド・チェックリスト（受領する Claude 向け）

- [ ] 第6章の表をデータ構造として一元化し、コードに二重定義していない。
- [ ] スクリプトはすべて標準ライブラリのみ（pip 依存ゼロ）。
- [ ] `docs-linter.py` が必須メタデータ・status 統制・id 整合・型/フォルダ整合・dead link・型規律を検出する。
- [ ] PostToolUse リンタが `continueOnBlock: true` で指摘を Claude に返す。
...
### 14.2 採用チェックリスト（各リポジトリで体系を導入する人向け）
...
- [ ] `review_by` を過ぎた 現行文書を点検した（`docs-audit`）。
...
各テンプレートは「共通フロントマター（第6.3節）＋型別本文骨格」で構成する。主要なものを示す。残りの型も同形式で作る。
...
（他の型 — PRODUCT, REQ, DATA, API, PRICING, ARCH, CTXMAP, CHANGE, IMPACT, COMPET, LEGAL, IMPL, TEST, RUNBOOK, INCIDENT, OPENQ, DEPREC, ARCHIVE, OVERVIEW, GLOSSARY — も同じ「共通フロントマター＋型別骨格」で作る。各型の「書く/書かない」は第6.1節表に従う。）
...
**アップロード（一次入力。本設計書がマージした二版）**
- `LLM開発情報管理体系.md`（概念枠組み版。24テンプレート、6観点成熟度、状態分離）
- `compass_artifact_...md`（経験的厳密版。7語統制語彙、リッチなメタデータ、14ステップ変更フロー、ETH 研究の限界提示）

**Claude Code 公式（実装機構の根拠。執筆時点で確認）**
- Hooks reference — https://code.claude.com/docs/en/hooks （イベント一覧、3つのカデンス、マッチャ規則、終了コード2、hookSpecificOutput、continueOnBlock、設置場所）
