...
文書を型と登録簿で整え、ガード・リンタ・投影で、LLM（大規模言語モデル）へ渡す情報を最小に保つClaude Codeプラグインである。設計書 `DOCTRINE-001`（[spec/doctrine.ja.md](https://github.com/Forest-Project-Lab/doctrine/blob/main/spec/doctrine.ja.md)）の参照実装にあたる。

本プラグインは設計書 `spec/doctrine.ja.md`（`DOCTRINE-001`）の実装である。設計書が要求（`R1`〜`R12`）と登録簿（型・状態・置き場所・必須キー）を定め、本プラグインがそれを再利用できる形（Skill・Hook・スクリプト・テンプレート）で機械的に強制する。設計書がモデルの正本であり、本プラグインは規則をコードに二重定義しない。規則の単一の出所は `scripts/_registry.py` と `templates/glossary.md.tmpl` にある。

`tests/` と `run_tests.py` は、CI と透明性のために同梱する。`plugin.json`・`hooks.json` はこれらを参照しないため、実行時には読み込まれない（配布物に含まれるが不活性）。設計書へのリンクは、配布時に相対パスが切れないよう GitHub の絶対 URL を使う。
...
- **Level 2（縮小構成）**: 必須キーだけ。型は `ICD`・`REQ`・`SPEC`・`ADR`・`DECIDED`・`OVERVIEW` に絞る。動くスクリプトは `docs-linter.py`・`policy-guard.py`（予防のみ）・`inject-contract.py` と共有コア。縮小は自主停止で効く: `scaffold.py --level 2` が `doctrine_docs/_system/.docs-level` に `level: 2` を書き、SessionEnd の監査・起動後ガード・レビューのナッジがこれを読んで静かに済ませる（ADR-019）。`hooks/hooks.level2.json` は、プラグインを使わず手で配線する場合の代替。
...
<!-- doctrine:view src=doctrine as-of=0.10.0 date=2026-08-03 refs=ICD-005,SPEC-016,SPEC-019,SPEC-020 -->
