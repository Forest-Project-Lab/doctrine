...
配布は `/plugin install` で行う。プラグインのHookは、パスを `${CLAUDE_PLUGIN_ROOT}/scripts/...` で解決する。

プラグインを使わない場合は、Skill `docs-system-init` が `.claude/`（プラグイン外の設定置き場）へフォールバック配置する。`scaffold.py` が `_system` の最小配置と段階導入の縮小構成（後述）を置く。既存のファイルは上書きしない。
...
- `PostToolUse` → `policy-guard.py` と `docs-linter.py`: ガードは `Edit`・`MultiEdit` の事後で、事前に判定できなかった `ICD` 依存違反・削除安全違反を再点検して止める。リンタは助言だけを返す（必須キー・状態の型別許可・id とファイル名の一致・型と置き場所の整合・`llm_context` の値・`SPEC` 必須4節・用語チェッカー・追跡性）。リンタは決して止めない。
...
| `scaffold.py` | `docs-system-init` | `_system` の最小配置（非破壊）。縮小構成を選べる |

`_frontmatter.py` と `_registry.py` は、二つ以上の入口が共有する中核である。先頭を `_` で始め、ハイフン名の入口スクリプトから読み込む。
