...
**著述・編集のたびに走らせる**。起動口は三つある。(1) doc-author の最後の手順がこのスキルを回す（手順 11）。(2) doc-author を介さない手編集には、PostToolUse の `review-nudge.py` がこのスキルを促す（§4.2。Level 3 以上）。(3) 定例は `gov-heartbeat.py` が周期超過を報せて促す。記載されれば動く、を既定にする。定例は、それでも残る `canonical_for` 未付与・辞書外の訳語臭・意味的重複の点検に充てる。

**指摘は定義の在処へ書き戻す**。一覧外のカルクは、導入先の用語辞書の運用正本（`_system/glossary.md` のカルク表）へ一行足す。用語チェッカーが読むのはこの運用正本であり、spec §1 は初期シードである。新しい承認語は ADR と用語辞書の更新をもって加える。型コードは登録簿（§3.2）、要求タグ（`[R番号]`）は §2 で既に定義済みとして扱い、辞書に二重定義しない。これにより、同じ指摘が次から機械的に閉じる。
...
5. 定例でだけ閉じる点検（運用契約、§7）。`canonical_for` 未付与・辞書外の訳語臭・意味的重複。周期の実体は `references/cadence-review.md` に定める（`_system/.governance-state` の実施記録と `gov-heartbeat.py` の督促で回す）。手動の依頼だけに頼らない。
6. 本当に新しい承認語が要るなら、`term-extract.py` の候補を証跡にして提案する。ただし `ADR` と用語辞書の更新を必須にする（§1）。自分で承認しない。新しいカルクは ADR を要さず、別ファイルも作らず、運用正本（`_system/glossary.md`）のカルク表に一行足す。
