...
下した判断は `doctrine_docs/_system/.md-intake` に一行で記録する。形式は `パス: 非文書|投影|保留 [YYYY-MM-DD]`。末尾 `/` の項目は配下全体に効く(例: `plugin/: 非文書`)。保留には期限を必ず書く。監査は、記録に無い .md を「未分類」(助言)として、期限を過ぎた保留を「再浮上」(警告)として挙げる。記録された非文書・投影は挙げない。これは検出を隠す除外の一覧ではなく、下した判断の記録である。記録しなければ、未決だけを機械で選り分けられない。記録の分類が正しいままかは、docs-curate の定例で見直す。
