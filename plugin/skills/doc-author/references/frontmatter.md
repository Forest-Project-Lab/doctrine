...
- `id` は `<型コード>-<連番>`。連番は、体系全体（全ドメイン）で見た同じ型の既存最大に 1 を足す。
- 欠番（割り当て済みの番号。例 SPEC-004・SPEC-005）は再利用しない。欠番は誤りではなく、開発の割り当ての跡である。
- 同じ型の連番はドメインを跨いで一意である（ADR-001〜 が複数ドメインに散在するのはこのため）。
