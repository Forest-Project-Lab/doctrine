...
- **hook-wiring**: 4 つのイベント（SessionStart・PreToolUse・PostToolUse・SessionEnd）を各スクリプトへ対応づける。matcher は `Edit|Write|MultiEdit` と `Bash` の二系統に分ける。PostToolUse では `policy-guard.py` を先に、`docs-linter.py` を後に起動する。
...
- **PostToolUse の起動順**: matcher `Edit|Write|MultiEdit` では、拒否しうる `policy-guard.py` を先に、助言だけを返す `docs-linter.py` を後に並べる。
- **縮小差分**: `hooks.level2.json` は、全構成から SessionEnd の `docs-audit.py` と PostToolUse の `policy-guard.py` を外したものである。
