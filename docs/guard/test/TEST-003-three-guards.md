...
- `TestEarlyOutNonDocs`: docs/ の外の純粋な非文書（.py/.txt）の編集は依存グラフを組まずに allow し（グラフ構築の呼び出し回数がゼロ）、docs/ 内の編集はグラフを組んで全ガードを当てる。フロントマターを持つ docs/外の Write は早期通過せず Guard2 が拒否する。docs/ 外のパス（external/…）やシンボリックリンク経由で `id` を持つ統治文書を降格する Edit は早期通過せず Guard3 が拒否する。`id` を持たないが `domain`＋越境 `depends_on` を持つ docs/外の文書型を編集した PostToolUse は早期通過せず Guard2 が block する。
